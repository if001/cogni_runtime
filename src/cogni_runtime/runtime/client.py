...
class AsyncClient:
    def __init__(self, runtime: AsyncMainAgentRuntime, max_queue: int = 1000) -> None:
        self.runtime = runtime
        self.sub_id, self._q = runtime.subscribe(max_queue=max_queue)
    ...
    async def await_final(self, turn_id: str, timeout: float = 120.0) -> str:
        t0 = asyncio.get_running_loop().time()
        while asyncio.get_running_loop().time() - t0 < timeout:
            ev: OutputEvent = await asyncio.to_thread(self._q.get)
            if ev.type == "assistant.final" and ev.turn_id == turn_id:
                return str(ev.payload.get("text", ""))
        raise TimeoutError(f"final not received: {turn_id}")
